...
(function($) {
	...
	var tips = [],
		reBgImage = /^url\(["']?([^"'\)]*)["']?\);?$/i,
		rePNG = /\.png$/i,
		ie6 = $j4.browser.msie && $j4.browser.version == 6;

	// make sure the tips' position is updated on resize
	function handleWindowResize() {
		$j4.each(tips, function() {
			this.refresh(true);
		});
	}
	$(window).resize(handleWindowResize);

	$j4.Poshytip = function(elm, options) {
		this.$elm = $(elm);
		this.opts = $j4.extend({}, $j4.fn.poshytip.defaults, options);
	...
	$j4.Poshytip.prototype = {
		init: function() {
			...
			switch (this.opts.showOn) {
				case 'hover':
					this.$elm.bind({
						'mouseenter.poshytip': $j4.proxy(this.mouseenter, this),
						'mouseleave.poshytip': $j4.proxy(this.mouseleave, this)
					});
					if (this.opts.alignTo == 'cursor')
						this.$elm.bind('mousemove.poshytip', $j4.proxy(this.mousemove, this));
					if (this.opts.allowTipHover)
						this.$tip.hover($.proxy(this.clearTimeouts, this), $j4.proxy(this.hide, this));
					break;
				case 'focus':
					this.$elm.bind({
						'focus.poshytip': $j4.proxy(this.show, this),
						'blur.poshytip': $j4.proxy(this.hide, this)
					});
					break;
			}
		},
	...
	$j4.fn.poshytip = function(options){
		...
		var opts = $j4.extend({}, $j4.fn.poshytip.defaults, options);
		...
		return this.each(function() {
			new $j4.Poshytip(this, opts);
		});
	}

	// default settings
	$j4.fn.poshytip.defaults = {
